...
1. OC 版本：0.78；
2. 当前配置只在 Catalina 10.15.7 上测试通过，其他版本不再测试；
3. 设备闲置，目前作为`爱奇艺`日常使用，博通网卡（Z3）用于其他主机，所以剔除博通网卡设置，改为 Intel 7265AC 的设置，上网蓝牙正常；
...
3. ~~没用过 Windows + macOS 双系统，所以产生的问题未知，也不会解决；~~
   ...
   在 Z490 主机尝试了双硬盘双系统，引导文件位于 Windows 所在磁盘，但目前还有问题：只要使用了 macOS，切换至 Windows 第一次启动必然蓝屏，需要再次启动才可以正常使用；

   有知道解决方案的大佬还望不吝赐教，先行感谢 🙏；
